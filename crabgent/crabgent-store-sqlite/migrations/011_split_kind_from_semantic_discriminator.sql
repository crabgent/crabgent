...
--
-- The `kind` column is the channel-kind (direct / group / im) and is not
-- touched. Rows with a non-`semantic` class are left alone even if the body
-- prefix happens to match (intentional: an `episodic` memory about a skill
-- stays episodic).
--
...
WHERE class = 'semantic'
  AND (body = '# user:' OR body LIKE '# user:%' || char(10) || '%');

UPDATE memory
SET class = 'notes'
WHERE class = 'semantic'
  AND (body = '# notes' OR body LIKE '# notes' || char(10) || '%');

UPDATE memory
SET class = 'skill'
WHERE class = 'semantic'
  AND (body = '# skill:' OR body LIKE '# skill:%' || char(10) || '%');

UPDATE memory
SET class = 'tools'
WHERE class = 'semantic'
  AND (body = '# tools' OR body LIKE '# tools' || char(10) || '%');
